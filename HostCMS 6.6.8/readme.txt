...
1) В панели администратора откройте вкладку Контент -> Интернет магазины -> "Название вашего магазина" -> вкладка "Справочники"->  Платежные системы -> сверху вкладка "Платежные системы" -> Добавить
	...
	*По желанию можете загрузить изображения из папки
...
2) В списке модулей зайдите на только что созданный модуль Интеркассы
	...
	Опуститесь ниже там есть семь главных параметра:
	protected $ik_co_id
	...
	Обратите внимание на $_interkassa_currency, посмотрите в "Финансы" -> "Валюты" на текущие валюты ыи убедитесь что утстановлен верный идентификатор валюты.
...
4)Переходим в корневую папку -> templates ->templateXX

5)Извлекаем содержимое архива template.zip в папку  templateXX

6) Еще раз проверьте правильно ли вы ввели все параметры и взяли нужную валюту
